...
;(function () {
  ...
  const _LEVEL = ['debug', 'info', 'warn', 'error']
  function createLogger(mode, report) {
    let rep = (...args) => report.forEach((r$1) => r$1(...args)),
      filter = (level, s) => (msg, e, scope) =>
      ...
      withScope = (scope) => ({
        ...
        timer: (label) => {
          ...
          return () => (
            (d = new Date()),
            rep(d, (d.getTime() - start).toFixed(2) + 'ms', 'timer', label)
          )
        },
        setLogMode: (m) => (mode = m),
      })
  ...
  let styleArray = []
  const logger = createBrowserLogger({ logMode: getDebug() ? 'debug' : 'disable' }).withScope(
    'scripts-mono',
  )
  ...
  const sansStyles = [`font-family:${getSettingsVariable('SANS')}`, 'letter-spacing:0px!important']
...
滚动条宽度: ${getScrollbarWidth()}
  `)
    ...
    _GM_registerMenuCommand(`设置 Monospace 字体特性`, () => {
      const monoSettings = prompt(
        'Monospace 字体特性 (https://developer.mozilla.org/zh-CN/docs/Web/CSS/font-feature-settings)',
        getMonoFeature(),
      )
      if (monoSettings) {
    ...
    _GM_registerMenuCommand(
      enableDark ? '适配黑暗模式（已启用）' : '适配黑暗模式（已关闭）',
      () => {
  ...
  const __vite_glob_0_4 = [
    (current2) => current2.endsWith('bilibili.com'),
    () => {
      addSansFont(
  ...
  const __vite_glob_0_9 = [
    (current2) => current2.endsWith('github.com') || current2.endsWith('bgithub.xyz'),
    () => {
      const imp = ' !important'
      ...
      addCSS(['.markdown-body', '.markdown-title', 'body'], sansStylesImportant)
  ...
  const __vite_glob_0_15 = [
    ...
    () => {
      addCodeFont(
        ...
        '.qwen-markdown-code .qwen-markdown-code-body .monaco-editor *',
      )
    },
  ]
  ...
  const __vite_glob_0_19 = [
    ...
    () => {
      addCSS(
        'body',
        ['--ff-sans:', '--ff-mono:monospace,'].map((s) => `${s}${getSans()}!important`),
      )
    },
  ]
  ...
  const __vite_glob_0_21 = [
    ...
    () => {
      addCSS(
        'div:is([lang=ja],[lang=en],[lang=ko])',
        `font-family:${getSettingsVariable('SANS')}!important;`,
      )
    },
  ]
  ...
  function loadSites(current2, customs) {
    const globs = Object.assign({
      './sites/2libra.ts': __vite_glob_0_0,
      './sites/51cto.ts': __vite_glob_0_1,
      './sites/affine.ts': __vite_glob_0_2,
      './sites/baidu.ts': __vite_glob_0_3,
      './sites/bilibili.ts': __vite_glob_0_4,
      './sites/cnblog.ts': __vite_glob_0_5,
      './sites/csdn.ts': __vite_glob_0_6,
      './sites/discord.ts': __vite_glob_0_7,
      './sites/gitee.ts': __vite_glob_0_8,
      './sites/github.ts': __vite_glob_0_9,
      './sites/jb51.ts': __vite_glob_0_10,
      './sites/jianshu.ts': __vite_glob_0_11,
      './sites/juejin.ts': __vite_glob_0_12,
      './sites/mdn.ts': __vite_glob_0_13,
      './sites/qqmail.ts': __vite_glob_0_14,
      './sites/qwen.ts': __vite_glob_0_15,
      './sites/raycast-website.ts': __vite_glob_0_16,
      './sites/regex101.ts': __vite_glob_0_17,
      './sites/sourcegraph.ts': __vite_glob_0_18,
      './sites/stackoverflow.ts': __vite_glob_0_19,
      './sites/tieba.ts': __vite_glob_0_20,
      './sites/twitter.ts': __vite_glob_0_21,
      './sites/v2ex.ts': __vite_glob_0_22,
      './sites/w3cschools.ts': __vite_glob_0_23,
      './sites/wechat.ts': __vite_glob_0_24,
      './sites/yuque.ts': __vite_glob_0_25,
    })
  ...
  const isDark =
    window.matchMedia('(prefers-color-scheme: dark)').matches && getSettings('DARK', false)
  if (isDark) {
